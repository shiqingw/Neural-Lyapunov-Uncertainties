...
# results_dir = '{}/results/exp_{:02d}_keep_eg1_nn_controller'.format(str(Path(__file__).parent.parent), exp_num)
...
plot_nested_roas_3D_diagnostic(grid, Tx, idx_small, None, plot_limits, plot_state_labels, 
                    ind_gap_stable=None, ind_exp_stable = forward_invariant, ind_true_roa=roa_true, 
                    full_path=os.path.join(results_dir, 'forward_invariant_after_training_iter_0.{}'.format(args.image_save_format_3d)))

c = nominal_c_max_exp_unconstrained_values[0]
idx_small = lyapunov_nn.values.detach().cpu().numpy().ravel() <= c
plot_nested_roas_3D_diagnostic(grid, Tx, idx_small, None, plot_limits, plot_state_labels, 
                ind_gap_stable=None, ind_exp_stable = None, ind_true_roa=roa_true,  
...
for k in range(args.roa_outer_iters):
    ...
    plot_nested_roas_3D_diagnostic(grid, Tx, idx_small, None, plot_limits, plot_state_labels, 
                    ind_gap_stable=None, ind_exp_stable = forward_invariant, ind_true_roa=roa_true, 
                    full_path=os.path.join(results_dir, 'forward_invariant_after_training_iter_{}.png'.format(k+1)))
    
    # c = nominal_c_max_exp_unconstrained_values[k+1]
    # idx_small = lyapunov_nn.values.detach().cpu().numpy().ravel() <= c
    # plot_nested_roas_3D_diagnostic(grid, Tx, idx_small, None, plot_limits, plot_state_labels, 
    #             ind_gap_stable=None, ind_exp_stable = None, ind_true_roa=roa_true,  
    #             full_path=os.path.join(results_dir, 'unconstrained_roa_after_training_iter_{}.png'.format(k+1)))
    

save_dict(post_proc_info, os.path.join(results_dir, "post_proc_info.npy"))