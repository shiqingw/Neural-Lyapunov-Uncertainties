...
def get_batch_grad(f, inputs):
    ...
    grads = torch.zeros(inputs.shape, dtype=config.ptdtype, device=config.device)
    if isinstance(inputs, torch.Tensor):
        inputs = inputs.detach().cpu().numpy()
    for i in range(nsamples):
        x = torch.tensor(inputs[[i]], requires_grad=True, dtype=config.ptdtype, device=config.device)
        y = f(x)
        grads[i] = torch.clamp(torch.autograd.grad(y, x, create_graph = True)[0], min=-10.0, max=10.0)
        x.requires_grad = False
    return grads


def get_batch_jacobian_norm(f, inputs):
    """"Computes the jacobian of a scalar-valued function
    with respect to its input
    ...
    grads : (nsamples, input dimension), The gradient of f
            with respect to each of its inputs.
    """
    inputs = torch.tensor(inputs, dtype=config.ptdtype, device=config.device)
    nsamples = inputs.shape[0]
    grads = torch.zeros_like(inputs, dtype=config.ptdtype, device=config.device)
    input_values = inputs.detach().numpy()
    for n in range(nsamples):
        x = torch.tensor(input_values[[n]], requires_grad=True, dtype=config.ptdtype, device=config.device)
        for i in range(2):
            tmp_grad = torch.autograd.grad(f, x, create_graph=True)[0]
            f = tmp_grad.sum()
        grads[n] = torch.norm(tmp_grad, p=1)
    return grads
...
def load_lyapunov_nn(lyapunov_nn, full_path):
    ...
    """
    loaded_state_dict = torch.load(full_path, map_location=config.device)
    lyapunov_nn.lyapunov_function.net.load_state_dict(loaded_state_dict)
    lyapunov_nn.lyapunov_function.net.to(config.device)
    return lyapunov_nn

def save_dynamics_nn(dynamics_nn, full_path):
    """
    Get an instance of Lyapunov and store the weights of its lyapunov_function which is a neural network
    """
    model = dynamics_nn
...
def load_dynamics_nn(dynamics_nn, full_path):
    """
    Get an initialized instance of Lyapunov and replace its lyapunov_function which is 
    a neural network with the loaded one from the specified address.
    """
    loaded_state_dict = torch.load(full_path, map_location=config.device)
    dynamics_nn.load_state_dict(loaded_state_dict)
    dynamics_nn.to(config.device)
    return dynamics_nn

def save_controller_nn(controller_nn, full_path):
    """
    Get an instance of Lyapunov and store the weights of its lyapunov_function which is a neural network
    """
    model = controller_nn
...
def load_controller_nn(controller_nn, full_path):
    """
    Get an initialized instance of Lyapunov and replace its lyapunov_function which is 
    a neural network with the loaded one from the specified address.
    """
    loaded_state_dict = torch.load(full_path, map_location=config.device)
    controller_nn.load_state_dict(loaded_state_dict)
    controller_nn.to(config.device)
    return controller_nn
