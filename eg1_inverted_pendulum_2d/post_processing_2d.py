...
from mars.visualization import plot_roa_2D, plot_levelsets, plot_nested_roas, plot_nested_roas_diagnostic
...
# plot_nested_roas_diagnostic(grid, Tx, idx_small, idx_big, plot_limits, plot_state_labels, 
#                     ind_gap_stable=None, ind_exp_stable = forward_invariant, ind_true_roa=roa_true, 
                    # full_path=os.path.join(results_dir, 'forward_invariant_after_training_iter_0.{}'.format(args.image_save_format)), add_lengend=True)
plot_nested_roas_diagnostic(grid, Tx, idx_small, idx_big, plot_limits, plot_state_labels, 
                    ind_gap_stable=None, ind_exp_stable = forward_invariant, ind_true_roa=roa_true, 
                    full_path=os.path.join(results_dir, 'forward_invariant_after_training_iter_0.eps'), add_lengend=True)
# assert(False)
c = nominal_c_max_exp_unconstrained_values[0]
idx_small = lyapunov_nn.values.detach().cpu().numpy().ravel() <= c
plot_nested_roas_diagnostic(grid, Tx, idx_small, roa_true, plot_limits, plot_state_labels, 
                ind_gap_stable=None, ind_exp_stable = None, ind_true_roa=None,  
                full_path=os.path.join(results_dir, 'unconstrained_roa_after_training_iter_0.{}'.format(args.image_save_format)))
...
# for k in range(args.roa_outer_iters):
    ...
    lyapunov_nn.update_values()
    c = nominal_c_max_exp_values[k+1]
    idx_small = lyapunov_nn.values.detach().cpu().numpy().ravel() <= c
    if args.roa_adaptive_level_multiplier:
    ...
    plot_nested_roas_diagnostic(grid, Tx, idx_small, idx_big, plot_limits, plot_state_labels, 
                    ind_gap_stable=None, ind_exp_stable = forward_invariant, ind_true_roa=roa_true, 
                    full_path=os.path.join(results_dir, 'forward_invariant_after_training_iter_{}.eps'.format(k+1)))
    
    c = nominal_c_max_exp_unconstrained_values[k+1]
    idx_small = lyapunov_nn.values.detach().cpu().numpy().ravel() <= c
    # plot_nested_roas_diagnostic(grid, Tx, idx_small, roa_true, plot_limits, plot_state_labels, 
    #             ind_gap_stable=None, ind_exp_stable = None, ind_true_roa=None,  
    #                 full_path=os.path.join(results_dir, 'unconstrained_roa_after_training_iter_{}.{}'.format(k+1, args.image_save_format)))
